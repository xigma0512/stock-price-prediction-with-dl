...
- 把MAPE改成MAE, RMSE，因為感覺MAPE有點難用來評估
- 我希望把關注點放在 **價格分布的特徵** ，所以我會希望把多支股票的價格數據控制在相近的分布範圍，讓模型專注在其分布的方式等
- 突然想到以前有學習過TPO，將價格根據某一個值，切分成很多個小方塊，再將這些小方塊全部往左移堆成一個分布圖，如果我有辦法將其量化成特徵給模型學習，不知道效果會怎麼樣。

### 25/09/24
- 我改了資料標準化的方式、模型的參數、訓練集/驗證集/測試集的切割方式，先備份到git當作一個進度...
    - 我對每一個品種的股票分別做了標準化，這樣可以把關注點放在 **價格的分布** 而不是 **價格的數值**
    - 我直接抓一支模型沒看過的股票來當測試集，這樣也可以驗證看看我的想法：**無論品種或週期，價格行為都是一樣的**